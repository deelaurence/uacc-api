...
const addWithdrawal = async (req, res) => {
    try {
        ...
        req.body.date = date;
        req.body.id = uuidv4();
        //add the amount Messageed to the total Messages field in the user schema
...
const getSingleWithdrawal = async (req, res) => {
    try {
        ...
        if (!singleWithdrawal) {
            throw new NotFound(
                `no Message with id ${withdrawalId} for ${req.decoded.name}`
            );
        }
...
const getWithdrawals = async (req, res) => {
    try {
        ...
        if (allWithdrawals.length < 1) {
            throw new NotFound("No Messages found for user");
        }
        res
...
const adminGetWithdrawals = async (req, res) => {
    try {
        if (req.query.q) {
            ...
            if (allWithdrawals.length < 1) {
                throw new NotFound("No Messages");
            }
...
const adminEditSingleWithdrawal = async (req, res) => {
    ...
    try {
        ...
        if (req.body.status == 'approved') {
            ...
            const finalMessageEdit = await Withdrawal.findOneAndUpdate({ id: withdrawalId }, { status: "approved", edited: true, })
            res.status(StatusCodes.OK).json(finalMessageEdit);
        }
        if (req.body.status == 'failed') {
            const finalMessageEdit = await Withdrawal.findOneAndUpdate({ id: withdrawalId }, { status: "failed", edited: true })
            res.status(StatusCodes.OK).json(finalMessageEdit);
        }
    } catch (error) {
        console.log(error.Message)
